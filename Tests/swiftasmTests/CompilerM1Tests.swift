...
final class CompilerM1Tests: XCTestCase {
    ...
    func testCompile_addressesAreAvailable() throws {
        let typeTable = SharedStorage(wrappedValue: [HLType.void])

        let compiledTable: SharedStorage<[HLCompiledFunction]> = SharedStorage(
            wrappedValue: [])
        ...
        typeTable.wrappedValue.append(
            HLType.fun(HLTypeFunData(args: [], ret: types.getResolvable(0 /*void*/)))
        )
    ...
    func testAppendPrologue() throws {
        let sut = OpBuilder()
        M1Compiler().appendPrologue(builder: sut)

        XCTAssertEqual(sut.build(), [0xfd, 0x7b, 0xbf, 0xa9, 0xfd, 0x03, 0x00, 0x91])
    }

    func testAppendEpilogue() throws {
        let sut = OpBuilder()
        M1Compiler().appendEpilogue(builder: sut)

        XCTAssertEqual(sut.build(), [0xfd, 0x7b, 0xc1, 0xa8])
    }
    ...
    func testGetRegStackOffset_min16() throws {
        ...
        XCTAssertEqual(0, M1Compiler().getRegStackOffset(regs, args: args, reg: 0))

        // Second reg is in stack after the register area,
        // which is aligned to 16 bytes
        XCTAssertEqual(16, M1Compiler().getRegStackOffset(regs, args: args, reg: 3))
    }
    ...
    func testGetRegStackOffset_noMin_funcArgs() throws {
        ...
        // Second reg is in stack after the register area,
        // which is aligned to 16 bytes
        XCTAssertEqual(4, M1Compiler().getRegStackOffset(regs, args: args, reg: 1))
    }
    ...
    func testAppendStackInit_skipVoid() throws {
        let mem = OpBuilder()
        try M1Compiler().appendStackInit([.void], args: [.void], builder: mem)
    ...
    func testAppendStackInit_min16() throws {
        ...
        let mem1 = OpBuilder()
        try M1Compiler().appendStackInit(_1_need16, args: _1_need16, builder: mem1)
        XCTAssertEqual(
            [
        ...
        let mem2 = OpBuilder()
        try M1Compiler().appendStackInit(_4_need16, args: _4_need16, builder: mem2)
        XCTAssertEqual(
            [
                0xff, 0x43, 0x00, 0xd1,  // sub sp, sp, #16
                0xe0, 0x03, 0x00, 0xf8,  // str x0, [sp, #0]
        ...
        let mem3 = OpBuilder()
        try M1Compiler().appendStackInit(_5_need32, args: _5_need32, builder: mem3)
        XCTAssertEqual(
            [
    ...
    func testAppendStackInit_multiple() throws {
        let mem = OpBuilder()
        try M1Compiler().appendStackInit(
            [.void, .i32, .i64],
            args: [.void, .i32, .i64],
            builder: mem
        )
    ...
    func testAppendStackInit_mismatchedRegs() throws {
        ...
        XCTAssertThrowsError(
            try M1Compiler().appendStackInit([.i32], args: [.void], builder: mem)
        )
    }

    func testAppendDebugPrintAligned4() throws {
        let sut = OpBuilder()
        M1Compiler().appendDebugPrintAligned4("Hello World", builder: sut)
