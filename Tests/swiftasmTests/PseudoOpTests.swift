...
final class PseudoOpTests: XCTestCase {
    ...
    func testMov_DeferredAbsoluteAddress() throws {
        @SharedStorage var deferred: UnsafeMutableRawPointer? = nil
        let op = PseudoOp.mov(.x0, _deferred)
        
        XCTAssertThrowsError(try op.emit())
        
        _deferred.wrappedValue = 1229801703818430600
        let res = try op.emit()
