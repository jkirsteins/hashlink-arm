...
fileprivate let ARG_REGISTER_COUNT = 8  // x0 through x7
...
func getRegStackOffset(_ regs: [HLType], args: [HLType], reg: Int) -> ByteCount {
    ...
    for ix in (skip..<reg) {
        result += ptr[ix].neededBytes
    }
    return result
}

/* SP movements must be aligned to 16-bytes */
func roundUpStackReservation(_ val: Int16) -> Int16 {
...
func calcStackArgReq(_ args: [HLType]) -> (Int16, ArraySlice<HLType>) {
    ...
    let result = stackArgs.reduce(0) { 
        $0 + Int16($1.neededBytes) 
    }
...
*/
func appendStackInit(_ unfilteredRegs: [HLType], args unfilteredArgs: [HLType], builder: OpBuilder) throws {
    // test mismatched before filtering
    let unfilteredArgRegCount = min(unfilteredArgs.count, ARG_REGISTER_COUNT)
    guard unfilteredRegs.prefix(unfilteredArgRegCount) == unfilteredArgs.prefix(unfilteredArgRegCount) else {
        throw GlobalError.expectationViolation("Up to first \(ARG_REGISTER_COUNT) registers must be the same for a function and its type.args")
    }
    ...
    builder.append(
        .sub(X.sp, X.sp, try .i(neededExtraStackSize))
    ) 
...
class M1Compiler {
    ...
    */
    func compile(
        ...
        appendDebugPrintAligned4("Entering function \(native.findex)@\(relativeBaseAddr.offsetFromBase)\n", builder: mem)
